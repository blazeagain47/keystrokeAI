...
export async function POST(req: NextRequest) {
  ...
  // A reconnecting host or known guest is always allowed through regardless
  // of status — they're reclaiming their own slot, not adding a new one.
  const isReconnect =
  ...
  if (blockedStatuses.includes(live.status as typeof blockedStatuses[number]) && !isReconnect) {
    return NextResponse.json(
  ...
  // Reject a new (third) player if the guest slot is already taken.
  if (live.guestId !== null && !isReconnect) {
